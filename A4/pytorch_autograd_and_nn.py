...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    self.conv1 = nn.Conv2d(in_channel, channel_1, kernel_size_1, padding=padding_1)
    self.conv2 = nn.Conv2d(channel_1, channel_2, kernel_size_2, padding=padding_2)
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin, Cout, kernel_size, padding=pad_size, stride=stride),
      nn.BatchNorm2d(Cout),
      nn.ReLU(),
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin, Cout // 4, kernel_size_1, stride=stride),
      nn.BatchNorm2d(Cout // 4),
      nn.ReLU(),
      nn.Conv2d(Cout // 4, Cout // 4, kernel_size_2, padding=pad_size),
      nn.BatchNorm2d(Cout // 4),
      nn.ReLU(),
      nn.Conv2d(Cout // 4, Cout, kernel_size_3)
    )
    if Cin == Cout:
      self.shortcut = nn.Identity()
    else:
