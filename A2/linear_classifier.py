...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # Replace "pass" statement with your code
  margins[margins > 0] = 1
  counts = torch.sum(margins, dim=1)
  margins[torch.arange(num_train), y] = -counts
  
  dW = X.t().mm(margins) / num_train + 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # Replace "pass" statement with your code
  cls.train(data_dict['X_train'], data_dict['y_train'], lr, reg, num_iters=num_iters)
